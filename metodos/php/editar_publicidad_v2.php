...
//    $archivo = $_FILES['archivo']['name'];
   $datos_usuario = 'titulo_publicidad='.$titulo_publicidad. '&texto='.$texto. '&fi='.$fecha_inicio.'&hi='.$hora_inicio.'&ff='.$fecha_final.'&hf='.$hora_final.'&fsuc='.$fk_sucursal.'&fdis='.$fk_dispositivo;


    if(empty($titulo_publicidad)){
        header("Location: ../formularios/editar_publicidad_v2.php?id=$id&error=titulo es requerido");
    }else if(empty($fecha_final)){
        header("Location: ../formularios/editar_publicidad_v2.php?id=$id&error=fecha final es requerido");
    }else if(empty($hora_final)){
        echo "La hora final esta vacia";
    }
    
    else if(empty($fecha_inicio)){
        header("Location: ../formularios/editar_publicidad_v2.php?id=$id&error=fecha inicio es requerido");

    }else if(empty($hora_inicio)){
        echo "La hora de inicio esta vacia.";
    }
    else if(empty($fk_sucursal)){
        header("Location: ../formularios/editar_publicidad_v2.php?id=$id&error=Sucursal es requerido");
    }else if(empty($fk_dispositivo)){
        header("Location: ../formularios/editar_publicidad_v2.php?id=$id&error=Dispositivo es requerido");
    }else if($fecha_inicio > $fecha_final){
        header("Location: ../formularios/editar_publicidad_v2.php?id=$id&error=Hora inicio es mayor a la final");
    }else{
        ...
        // var_dump($tipo);
        echo "Editar publicidad hoy aun esta en modificacion";
